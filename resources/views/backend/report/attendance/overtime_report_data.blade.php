...
@section('title', 'Overtime Report Data')
@section('content')
            ...
            <div class="col-lg-12">
                <div class="panel panel-blue">
                    <div class="panel-heading">
                        <div class="row">
                            <div class="col-md-6">
                                Overtime Report From {{\Carbon\Carbon::parse($request->date)->format('j M Y')}} to {{\Carbon\Carbon::parse($request->end_date)->format('j M Y')}}
                            </div>
                            {!! Form::open(['target'=>'_blank','method'=>'post','action'=>'AttendanceReportController@overtime_report_export']) !!}
                    ...
                    <div class="panel-body table-responsive">
                        <table id="overtime_report_table" class="table table-striped table-bordered" >
                            <thead>
                            <tr>
                                <th width="5%">No.</th>
                                <th width="15%">Name</th>
...
@section('extra_js')
    <script>
        $(document).ready(function(){
            $('#overtime_report_table').DataTable({
                processing: true,
                serverSide: true,
                "order": [[ 0, "desc" ]],
                ajax:{
                    url: "{{ route('attendance.overtime_report_data') }}",
                    type:"post",
                    data:{
                        "_token": "{{ csrf_token() }}",
                        'date':"{{$request->date}}",
