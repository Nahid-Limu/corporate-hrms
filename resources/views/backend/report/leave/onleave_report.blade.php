...
<head>
    ...
    <title>On Leave Employee Report</title>
    <style type="text/css">
        *{
            padding: 0;
            margin: 0;
    ...
    <table class="table2 table-bordered"  style="width:100%">
        ...
        @foreach($leave as $key=>$leave_employee)
            <tr>
                ...
                <td>{{$leave_employee->leave_type}} ({{$leave_employee->total_days}})</td>
                <td>{{$leave_employee->total}}</td>
                <td>{{$leave_employee->availabe_leave}}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
